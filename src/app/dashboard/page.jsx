...
const adminEmail = "your@email.com"; // CHANGE THIS to your admin email
...
const Dashboard = () => {
  ...
  const [editDueDateId, setEditDueDateId] = useState(null);
        ...
        const workspaceData = await Promise.all(
          querySnapshot.docs.map(async (workspaceDoc) => {
            ...
            return {
              id: workspaceDoc.id,
              ...workspaceDoc.data(),
              createdAt: workspaceDoc.data().createdAt ? new Date(workspaceDoc.data().createdAt.seconds ? workspaceDoc.data().createdAt.seconds * 1000 : workspaceDoc.data().createdAt) : null,
              lastEdited: workspaceDoc.data().lastEdited ? new Date(workspaceDoc.data().lastEdited.seconds ? workspaceDoc.data().lastEdited.seconds * 1000 : workspaceDoc.data().lastEdited) : null,
              dueDate: workspaceDoc.data().dueDate ? new Date(workspaceDoc.data().dueDate.seconds ? workspaceDoc.data().dueDate.seconds * 1000 : workspaceDoc.data().dueDate) : null,
  ...
  const handleEditDueDate = (ws) => {
    setEditDueDateId(ws.id);
    setEditDueDateValue(ws.dueDate ? dayjs(ws.dueDate).format("YYYY-MM-DD") : "");
  };
  const saveDueDate = async () => {
    if (!editDueDateId) return;
    try {
      const dueDateToSave = editDueDateValue ? new Date(editDueDateValue) : null;
      await setDoc(doc(db, "workspaces", editDueDateId), { dueDate: dueDateToSave, lastEdited: new Date() }, { merge: true });
      setWorkspaces((prev) => prev.map(ws => ws.id === editDueDateId ? { ...ws, dueDate: dueDateToSave, lastEdited: new Date() } : ws));
      toast.success("Due date updated!", toastOptions);
    } catch (e) {
      toast.error("Failed to update due date.", toastOptions);
    }
  ...
  const updateAllWorkspaces = async () => {
    try {
      ...
      for (const docSnap of querySnapshot.docs) {
        ...
        let updateObj = {};
        if (!data.createdAt) updateObj.createdAt = now;
        if (!data.lastEdited) updateObj.lastEdited = now;
        if (Object.keys(updateObj).length > 0) {
          await setDoc(doc(db, "workspaces", docSnap.id), updateObj, { merge: true });
          updatedCount++;
        }
      }
      toast.success(`Updated ${updatedCount} workspaces!`, toastOptions);
    } catch (e) {
      toast.error("Failed to update workspaces.", toastOptions);
    }
  };
    ...
    <div className="h-screen w-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white flex flex-col animate-gradient">
      ...
      <div className="flex justify-between items-center p-6">
        ...
        {user?.email === adminEmail && (
          <Button onClick={updateAllWorkspaces} className="ml-4 bg-green-600 text-white px-4 py-2 rounded shadow">Update All Workspaces</Button>
        )}
            ...
            {workspaces.length === 0 ? (
              ...
              workspaces.map((ws) => (
                ...
                >
                  <CardContent className="p-6 flex flex-col gap-4">
                    <Link href={`/workspace/${ws.id}`} className="block">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-col gap-1 text-xs font-mono text-yellow-400">
                          {ws.lastEdited && <span>Last edited: {dayjs(ws.lastEdited).fromNow()}</span>}
                          {ws.createdAt && <span>Created on: {dayjs(ws.createdAt).format('MMM D, YYYY')}</span>}
                          {ws.dueDate && <span>Due Date: {dayjs(ws.dueDate).format('MMM D, YYYY')}</span>}
                        </div>
                        <Button size="sm" className="mt-2 bg-gray-500 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs w-max" onClick={e => { e.preventDefault(); e.stopPropagation(); handleEditDueDate(ws); }}>
                          Edit Due Date
                        </Button>
                  ...
                  {/* Robot icon at right bottom corner */}
                  <span className="absolute bottom-3 right-4 text-2xl select-none pointer-events-none">🤖</span>
                </Card>
              ))
            )}
          </div>
      ...
      {editDueDateId && (
        <Dialog open={true} onOpenChange={() => setEditDueDateId(null)}>
          <DialogContent className="bg-[#1E293B] text-white">
            <DialogTitle>Edit Due Date</DialogTitle>
            <DialogDescription>Pick a new due date for this workspace.</DialogDescription>
            <input
              type="date"
            ...
            />
            <div className="flex space-x-4">
              <Button onClick={saveDueDate} className="bg-green-500 text-white">Save</Button>
              <Button onClick={() => setEditDueDateId(null)} className="bg-gray-500 text-white">Cancel</Button>
            </div>
          </DialogContent>
        </Dialog>
      )}
